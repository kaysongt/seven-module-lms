...
import { getTrainingVideo, requiresVideoCompletion } from "@/lib/video-progress";
...
export default async function LessonPage({ params }: { params: Promise<{ moduleSlug: string; lessonSlug: string }> }) {
  ...
  const needsVideo = requiresVideoCompletion(lesson);
      ...
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[270px_1fr]">
        ...
        <article className="min-w-0 px-4 py-10 md:px-10 md:py-14 xl:px-20">
          <div className="mx-auto max-w-3xl">
            <div className="flex flex-wrap items-center gap-4 text-[0.68rem] font-extrabold tracking-[0.08em] text-[var(--ink-soft)]/60 uppercase"><span>{lesson.kind}</span><span className="inline-flex items-center gap-1"><Clock3 size={13} /> {video ? Math.ceil(video.duration / 60) : lesson.estimatedMinutes} min</span>{lesson.isComplete && <span className="inline-flex items-center gap-1 text-[var(--forest)]"><Check size={13} /> Complete</span>}</div>
            {video && <TrackedVideo lessonId={lesson.id} videoId={video.id} duration={video.duration} title={lesson.title} initialWatched={lesson.watchedSeconds} initialComplete={lesson.isComplete} />}
            {needsVideo && !video && <p role="alert" className="mt-8 rounded-xl border border-[var(--line)] p-5">This teaching is not available yet. Please contact your instructor; the next lesson will unlock after the teaching can be completed.</p>}
            ...
            <div className="mt-12 flex flex-wrap items-center justify-between gap-4 border-t border-[var(--line)] pt-7">
              {!lesson.isComplete && !needsVideo && lesson.kind !== "ASSIGNMENT" && <form action={completeAction}><button className="button-primary"><Check size={17} /> Mark complete</button></form>}
              {!lesson.isComplete && needsVideo && <span className="text-sm font-bold text-[var(--ink-soft)]">Finish the teaching above to unlock the next lesson.</span>}
